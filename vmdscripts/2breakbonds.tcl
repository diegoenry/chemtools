...
#find shell atoms on the surface
...
######################################################################## Part 2
# OK, now find shell atoms on the VERY surface
    set gridsz  1                       ; # grid spacing
    set radius  6                       ; # sphere radius 
    set dist    2                       ; # distance from surface
    set shellatoms      [measure surface $all $gridsz $radius $dist] 
    set shell           [atomselect top [concat "index" $shellatoms]]
    set interior        [atomselect top [concat "not index" $shellatoms]]
    set num_shell       [$shell num]
    set num_interior    [$interior num]
#create the shell and interior selections
    puts "Labeled $num_shell surface atoms with beta = 0"
    puts "Labeled $num_interior interior atoms with beta = 1" 
#fixing selection !keep this order !
    set fix  $interior
    set free $shell
    $fix  set beta 1
    ...
    $all writepdb ok2breakbonds.pdb

    puts  "Atoms within 4 Angstrons from vacuum are considered as part of the surface."
    ...
    puts  " "
    puts  "I wrote a ok2breakbonds.pdb for you, atoms with beta = 0 can have bonds "