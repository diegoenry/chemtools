...
#create list of the NON dangling OXYGEN atoms on the surface (beta 0)
    ...
    set j 0
    for {set i 0} {$j < $nbonds_to_delete} {incr i} {
# generate random number within the range of the selection  
        set delete_this [ lindex $ndo_bonds [lindex $newlist $i] 1 ]
        if { [in $ndsi_index $delete_this] } {
            puts "Deleting [ lindex $ndo_index [lindex $newlist $i] ] $delete_this"
            topo delbond [ lindex $ndo_index [lindex $newlist $i] ] $delete_this"
            set ndsi [atomselect top "type SI and beta 0 and numbonds 4"]   ;# recount the bonds, one of the silicons on the list now has 3bonds. 
            set j [ expr $j +1 ]
        }
    }
